import pygame, random, time, math, turtle, sys # import all related modules
...
dont forget to init ALL sprites at the beginning -> no overlap/flutter
"""
...
class BossHealth(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.images = []
        for i in range(1, 7):
            img = pygame.image.load("bhealth"+str(i)+".png").convert()
            img.convert_alpha()
            img.set_colorkey(WHITE)
            self.images.append(img)
        self.image = self.images[0]
...
#GROUND BOSS enemy sprite class
class Shell(pygame.sprite.Sprite):
    def __init__(self):
        ...
        center = self.rect.center # find center
        # scale down sprite and cast to int to scale properly
        self.image = pygame.transform.scale(self.image, ((int)(factor6*self.rect.width), (int)(factor6*self.rect.height))) # scale down sprite
    ...
    def update(self): #counter to move enemy animations slower
        global boss_appear, shell_set
        if boss_appear and shell_set:
            self.image = self.images[0]
            self.rect = self.image.get_rect()
            self.rect.x = 749
            self.rect.y = 420
        ...
        self.hitbox = self.rect.inflate(-45, 0)
        #print (self.rect.x, self.rect.y, "rect")
        global shell_lr, shell_move # use global values
        #Check x boundaries --> only moves left and right
        if self.rect.x < 0:
            self.rect.x = 750
            shell_lr = random.randint(3, 10)
            shell_move = True
        #scaling
        """
        global factor6
...
class Bomb(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.images = []
        for i in range(1, 3):
            img = pygame.image.load("bomb"+str(i)+".png").convert()
            img.convert_alpha()
            img.set_colorkey(WHITE)
            self.images.append(img)
        self.image = self.images[1]
...
class Boss(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.images = []
        for i in range(1, 3):
            img = pygame.image.load("boss"+str(i)+".png").convert()
            img.convert_alpha()
            img.set_colorkey(WHITE)
            self.images.append(img)
        self.image = self.images[1]
    ...
    def update(self):
        ...
        if boss_appear:
            ...
            self.rect.x = self.rect.x + self.movex
            self.rect.y = self.rect.y + self.movey
            global boss_lr, boss_move, boss_dir
            #Check x boundaries --> only moves left and right
            if self.rect.x > 730:
                self.rect.x = 730
                boss_lr = random.randint(3, 8)
                boss_move = True
                boss_dir = False
            if self.rect.x < 0:
                self.rect.x = 0
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.rect.x > 730:
            ...
            self.rect.y = 50
        #advance animation frames
        global ani, invincible # use global animation frames
        #moving left
        if self.movex < 0:
            self.frame += 1
            ...
            if invincible:
                ...
                self.image = self.images[(self.frame//ani)]
            self.image = pygame.transform.flip(self.image, True, False) #lip image
        # moving right
        if self.movex > 0:
            self.frame += 1
            if self.frame >= 6*ani: # number of frames * ani
...
# collided function to check for collision with other sprites
...
def start_text():
    ...
    text = font.render("Super Mugman Mario Knight", 0, (10, 10, 10))
    screen.blit(text, (50, 100))
    text = font.render("Made by Jerry Zhu and Completed On January 20th", 0, (10, 10, 10))
    screen.blit(text, (50, 150))
    text = font.render("Press s to start. ", 0, (10, 10, 10))
    screen.blit(text, (50, 200))
    text = font.render("Objective: Survive as long as possible and defeat the boss. ", 0, (10, 10, 10))
    screen.blit(text, (50, 250))
    text = font.render("Use WASD or arrow keys to move and press Q to dash. ", 0, (10, 10, 10))
    screen.blit(text, (50, 300))
    text = font.render("Pick up bombs and powerups and dodge all other enemies. ", 0, (10, 10, 10))
...
def almost_text():
    font = pygame.font.Font(None, 36)
    text = font.render("So Close! Have another go... U CAN DO THIS!", 0, (10, 10, 10))
    screen.blit(text, (150, 100))
    text = font.render("Press f to play again. ", 0, (10, 10, 10))
    screen.blit(text, (150, 200))
    text = font.render("Press d to stop playing. ", 0, (10, 10, 10))
...
def win_text():
    font = pygame.font.Font(None, 36)
    text = font.render("You Win! Thank you for playing :)", 0, (10, 10, 10))
    screen.blit(text, (150, 100))
    text = font.render("Press f to play again. ", 0, (10, 10, 10))
    screen.blit(text, (150, 200))
    text = font.render("Press d to stop playing. ", 0, (10, 10, 10))
...
# didn't get past boss gameover screen
def die():
...
#get past boss gameover screen
def almost():
    pygame.mixer.music.load("almost.wav")
    pygame.mixer.music.play(-1)
    screen.fill(WHITE)
    almost_text()
    pygame.display.flip() # change the display screen
...
def game_loop():
    ...
    global enemy_lr, enemy_move, game_over, dash_amount, isjump, isdash, jump_start, jumpcount, dash_cnt, jump_start, jumpcount \
           ...
           koopa_type, koopa_cnt, invincible, invincible_cnt, powerup_type, final_cnt, boss_appear, boss_dir, boss_move, boss_lr, \
           shell_move, shell_lr, win_game, bomb_lr, bomb_show
    done = True
    #start theme music
    ...
    while done:
        ...
        for event in pygame.event.get():
            # if you have to quit the program, quit it
            if event.type == pygame.QUIT: 
                pygame.quit()
                quit()
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if (event.key == pygame.K_UP or event.key == ord('w'))and not isjump:
                    isjump = True
                #dashing through the snow -> check for dashing with Q
        ...
        #jumping: parabolic movement with realistic delay frames
        if isjump:
        ...
        if isdash:
            ...
            if dash_cnt == 2:
                ...
                if dash_dir:
                    ...
                    player.control(-dash_lr, 0)
        # move all sprites on the screen by using sprite/object detection 
        if enemy_move:
            #print(enemy_lr, "hi")
        ...
        if lakitu_move:
            if lakitu_dir:
                ...
                lakitu.control(-lakitu_lr, 0)
            lakitu_move = False
        #special for boss objects
        if boss_appear:
            if boss_move:
                if boss_dir:
                    boss.control(boss_lr, 0)
                else:
                    boss.control(-boss_lr, 0)
                boss_move = False
            if shell_move:
                shell.control(-shell_lr, 0)
                shell_move = False
            if bomb_show:
                bomb.show()
                bomb.control(0, bomb_lr)
                bomb_show = False
        if egg_show:
            egg.show()
        ...
        final_cnt += 1
        if final_cnt == 1000: # when the boss will appear(you can change this to make the game easier/harder)
            boss_appear = True
            health.start()
        ...
        if not invincible:
            ...
            if boss_appear:
                shell_hit_list = []
                shell_hit_list = pygame.sprite.spritecollide(player, shell_list, False, collided) # checks for player and enemy hitboxes
                if not len(shell_hit_list) == 0:
                    player.revert()
                    shell.revert()
                    health.revert()
                    if game_over == True:
        ...
        if boss_appear:
            bomb_hit_list = []
            bomb_hit_list = pygame.sprite.spritecollide(player, bomb_list, False, collided) # checks for player and enemy hitboxes
            if not len(bomb_hit_list) == 0:
                bomb.revert()
                bhealth.revert()
                if win_game == True:
                    #play win sound
...
def init_variables():
    ...
    koopa_type, koopa_cnt, invincible, invincible_cnt, powerup_type, final_cnt, boss_appear, boss_dir, boss_move, boss_lr, shell_move, shell_lr, factor6, \
    shell_set, win_game, bomb_lr, bomb_show
    #variable inits
    factor = 0.6
    factor2 = 0.5
    factor3 = 0.7
    ...
    boss_appear = False
    boss_lr = random.randint(3, 8)
    boss_move = True # moving or not?
    boss_dir = True # moving right
    shell_lr = random.randint(3, 10)
    shell_move = True
    shell_set = True
    win_game = False
...
def init_sprites():
    ...
    health.rect.y = 0 # health bar y value
    bhealth.rect.x = 200
    bhealth.rect.y = 0
    powerup.rect.x = random.randint(100, 400)
    powerup.rect.y = 420
    powerup.hitbox = powerup.rect
    koopa.type = 1
    powerup.frame = 0
    powerup.cnt = 0
...
global shell_move
global shell_set
global win_game # if u've won da game
# bomb commands
global bomb_show
global bomb_lr
...
bhealth_list = pygame.sprite.Group()
